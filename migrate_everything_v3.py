...
def get_or_create_account(name):
    name = str(name).strip()
    if name == 'None' or not name: return env['account.account'].browse(1) # Safety
...
def migrate():
    ...
    # CLEAR EXISTING FROM THIS TURN FOR A CLEAN START
    # (Optional, but helps during debug)
    # env['account.move'].search([('ref', 'like', 'Bank Payment')]).unlink()
    ...
    for reg in registers:
        ...
        try:
            ...
            for i, row in enumerate(ws.iter_rows(values_only=True)):
                ...
                if i < 10: continue # Skip Row 9 headers and Row 10 sub-headers
                ...
                if reg['type'] == 'columnar':
                    ...
                    for k,v in row_data.items():
                        if k in ['Date', 'Particulars', 'Voucher Type', 'Vch Type', 'Vch No.', 'Voucher No.', 'Gross Total', 'Value']: continue
                        if v and float(v) != 0:
                            v = float(v)
                            o_acc = get_or_create_account(k)
                            lines.append((0,0, {'account_id': o_acc.id, 'debit': v if v > 0 else 0, 'credit': -v if v < 0 else 0, 'name': f"{v_type} {v_no}"}))
                    # Balance it
