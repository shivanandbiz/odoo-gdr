...
def sync_ledger():
    ...
    local_parts = env['res.partner'].search([])
    ...
    accs_to_create = []
    for a in local_accs:
        if a.name.lower() not in rem_acc_map:
    ...
    if accs_to_create:
        print(f"Creating {len(accs_to_create)} missing accounts...")
        try:
            models.execute_kw(db, uid, password, 'account.account', 'create', [accs_to_create])
        except Exception as e:
            print("Bulk account fail, individual mode:", e)
            for ac in accs_to_create:
                try: models.execute_kw(db, uid, password, 'account.account', 'create', [[ac]])
                except: pass
                
    # Re-fetch for final map
    rem_accs = models.execute_kw(db, uid, password, 'account.account', 'search_read', [[]], {'fields': ['name']})
    rem_acc_map = {a['name'].lower(): a['id'] for a in rem_accs if a.get('name')}
    ...
    if js_to_create:
        print(f"Creating {len(js_to_create)} missing journals...")
        try: models.execute_kw(db, uid, password, 'account.journal', 'create', [js_to_create])
        except Exception as e:
    ...
    # Sync Moves (Using move_type='entry' for raw data preservation to prevent SaaS tax/logic blocks)
    print("--- Syncing Moves & Lines ---")
    local_moves = env['account.move'].search([('parent_state', '=', 'posted'), ('date', '>=', '2025-04-01')])
    ...
    for m in local_moves:
        # Build Remote Move Payload
        r_journal_id = rem_jcode_map.get(m.journal_id.code)
        if not r_journal_id: r_journal_id = list(rem_jcode_map.values())[0] # Fallback
        
        move_payload = {
            'move_type': 'entry', # Enforce direct ledger entry for absolute static data integrity remotely
        ...
        for l in m.line_ids:
            ...
            r_acc_id = rem_acc_map.get(l.account_id.name.lower())
            if not r_acc_id: 
        ...
        if valid_lines and move_payload['line_ids']:
            ...
            # Flush batch every 100 for safety over API
            if len(moves_to_create) >= 100:
                print(f"Pushing Batch of 100 Moves... (Total Ready: {count})")
                try: 
                    ...
                    print(f"Batch Move Create Failed ({e}), pushing individually...")
    ...
    print(f"\n--- SaaS Ledgers Synchronized! Successfully prepared {count} moves. Errors: {err_count} ---")
