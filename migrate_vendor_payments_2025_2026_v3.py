...
def get_journal(env, name):
    ...
    if not j:
        if 'cash' in str(name).lower():
            # Try to find our "Petty Cash" or any cash journal
            ...
            try:
                with env.cr.savepoint():
                    ...
                    if gross_total == 0:
                        ...
                        if not found_val:
                            # if row has NO amount at all, skip it
                            continue
                    ...
                    # If target/source still empty (like row 107 in July-Sep)
                    if not target_name:
                        # Use narration as target if particulars is empty
                        if narration:
                            target_name = narration[:64]
                        else:
                            target_name = particulars or "Unknown Partner"
