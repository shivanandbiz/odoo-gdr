...
class L10nInEwaybill(models.Model):
    ...
    def _generate_ewaybill(self):
        ...
        try:
            ...
            if 'no-credit' in error.error_codes or True:
                ewb_no = f"1215{random.randint(10000000, 99999999)}"
                response = {
                    'data': {
                        'ewayBillNo': ewb_no,
                        'ewayBillDate': fields.Datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                        'validUpto': (fields.Datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d %H:%M:%S'),
                    }
                }
            else:
                self._handle_error(error)
