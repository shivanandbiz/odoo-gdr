...
class AccountMove(models.Model):
    ...
    def _has_reconciled_invoices_with_different_currency(self):
        self.ensure_one()
        partials = self._get_reconciled_invoices_partials()[0]
        return any(self.currency_id != par[2].currency_id for par in partials)

    def _has_long_line_description(self):
        self.ensure_one()
        return any(len(l.name.split('\n')) > 30 for l in self.invoice_line_ids if l.name)

    def _get_invoice_lines_sorted(self):
        self.ensure_one()
        return self.invoice_line_ids.sorted(key=lambda l: (-l.sequence, l.date, l.move_name, -l.id), reverse=True)
