"""
Import vendors from vendor_od00.xlsx into Odoo res.partner

Run via:
  /home/biz/odoo/odoo-venv/bin/python3 /home/biz/odoo/odoo-bin shell \
      --config /home/biz/odoo/odoo.conf \
      --no-http \
      -d Odoo < /home/biz/odoo/import_vendors.py 2>&1 | tee /home/biz/odoo/import_vendors.log
"""
...
# ── Load Excel ────────────────────────────────────────────────────────────────
file_path = '/home/biz/odoo/vendor_od00.xlsx'
df = pd.read_excel(file_path, engine='openpyxl')

# Remove sub-rows (rows without a complete_name)
df_main = df[df['complete_name'].notna()].copy().reset_index(drop=True)
print(f"Total main vendor rows: {len(df_main)}")
...
def get_state(name, country_id):
    ...
    # Strip (XX) if present, e.g., "Karnataka (IN)" -> "Karnataka"
...
for idx, row in df_main.iterrows():
    try:
        ...
        salesperson_name = str_clean(row.get('user_id'))
        ...
        # If we have an external ID, we could also use that, but name is usually safer for matching existing.
        # However, if Odoo 19 uses 'id' as a string external id, we might look it up.
        # For now, stick to Name and VAT.
        ...
        print(f"  [ERROR] Row {idx} ({str_clean(row.get('complete_name'))}): {e}")
        env.cr.rollback()
        # Clear caches
        country_cache.clear()
        state_cache.clear()
