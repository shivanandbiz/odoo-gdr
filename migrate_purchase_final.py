# migrate_purchase_final.py
# Migrates Purchase Register from Excel to Odoo.
# Logic:
# - April 2025 to January 2026: From all_tally_to_odoo_migratation.xlsx
# - February 2026 to March 2026: From purchase.xlsx-2025-26.......xlsx
# - For January, skip early rows that were found to be duplicates (index < 800).
# - Handles GST breakdown (18%, 12%, 5%, 28%).
# - Matches Grand Total: 9,03,23,140.32
...
def read_xlsx(fname, sheet_name='Purchase Register'):
    ...
    for idx, row in enumerate(rows[hdr_idx + 1:]):
        ...
        try:
            ...
            # Fallback for shifted columns in March File 1
            try:
                d['_gross'] = float(d.get('LOCAL PURCHASE GST 18%') or 0)
            except: d['_gross'] = 0.0
        data.append(d)
    return data
...
for r in rows1:
    ...
    if my == '2026-01' and r['_row_idx'] < 800: continue
    if my <= '2026-01': final_rows.append(r)

for r in rows2:
    my = r['_date'].strftime('%Y-%m')
    if my in ['2026-02', '2026-03']: final_rows.append(r)

print(f"Total records to migrate: {len(final_rows)}")

# ── 2. DELETE existing purchase invoices ────────────────────────────────────
...
for idx, row in enumerate(final_rows):
    ...
    ref = inv_no if inv_no and inv_no != 'nan' else f'MIG/{idx}'

    # Tax detection - only include columns that represent ACTUAL TAX AMOUNTS
...
print(f"Tally Grand Total: 9,03,23,140.32")
