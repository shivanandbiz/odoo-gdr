...
def migrate_final_v4():
    ...
    for idx, row in df.iloc[9:].iterrows():
        ...
        try:
            ...
            is_bank_transfer = any(part_name.lower() in val.lower() for val in dest_map.values())
            ...
            if is_bank_transfer:
                # Still use the actual bank account for the move
                source_acc = get_account(part_name)
                source_acc_id = source_acc.id if source_acc else partner.property_account_receivable_id.id
            else:
            ...
            # 2. Create Payment Record (FORCE AS CUSTOMER PAY TO SHOW IN LIST)
            payment = env['account.payment'].create({
                'payment_type': 'inbound',
                'partner_type': 'customer', # Force this so it shows in the user's list
                'partner_id': partner_id,
                'amount': amount,
                'journal_id': journal.id,
                'date': dt_str,
    ...
    env.cr.commit()
    print(f"\nFinal Migration v4 Finished.")
    print(f"Total Migrated (Visible in list): {count} | Reconciled: {reconciled_count} | Errors: {errors}")
