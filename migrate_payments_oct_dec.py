...
def migrate_oct_dec_payments():
    fname = '/home/biz/odoo/payment_register_oct_dec.xlsx'
    ...
    print("Cleaning up older migration data (ref like PAY_OCT_DEC/)...")
    env['account.move'].search([('ref', 'like', 'PAY_OCT_DEC/%')]).button_draft()
    env['account.move'].search([('ref', 'like', 'PAY_OCT_DEC/%')]).unlink()
    env['account.payment'].search([('memo', 'like', 'PAY_OCT_DEC/%')]).action_draft()
    env['account.payment'].search([('memo', 'like', 'PAY_OCT_DEC/%')]).unlink()
    env.cr.commit()
    ...
    for idx, row in df.iloc[9:].iterrows():
        ...
        is_bank_source = any(b in part_name for b in ['HDFC', 'Kotak', 'Bank', 'Cash', 'CASH', 'THE KARUR'])
        ...
        if is_bank_source:
            ...
            dr_name = part_name
            for name, val in vals.items():
                if any(b in name for b in ['HDFC', 'Kotak', 'Bank', 'Cash', 'CASH', 'THE KARUR']):
                    cr_name = name
                    amount = abs(val)
                    break
            if not cr_name:
        ...
        try:
            ...
            payment = env['account.payment'].create({
                ...
                'journal_id': journal.id,
                'date': dt_str,
                'memo': f"PAY_OCT_DEC/{vref} | {narration}",
                'state': 'in_process',
            })
