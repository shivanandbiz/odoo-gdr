    ...
    for sheet in sheet_names:
        ...
        for idx, row in df.iloc[data_start_idx:].iterrows():
            ...
            try:
                ...
                # USE SAVEPOINT for action_post()
                try:
                    with env.cr.savepoint():
                        move.action_post()
                except Exception as pe:
                    print(f"  Row {idx} could not post move: {pe}")
                    continue # Skip if we can't post
        ...
        env.cr.commit()
    print(f"\nFINAL SUCCESS: Total {total_count} | Reconciled {total_reconciled} | Errors {errors}")
