...
def migrate_gdr_payments():
    ...
    for pref in prefixes:
        ...
        if moves:
            print(f"  Cleanup {len(moves)} moves {pref}")
        ...
        if payments:
            print(f"  Cleanup {len(payments)} payments {pref}")
            payments.filtered(lambda p: p.state != 'draft').action_draft()
            payments.unlink()
    
    # Force clean up reconciles for the period
        ...
        for idx, row in df.iloc[data_start_idx:].iterrows():
            ...
            try:
                ...
                payment = env['account.payment'].create({
                    'payment_type': 'outbound', 'partner_type': 'supplier', 'partner_id': partner.id if partner else False,
                    'amount': amount, 'journal_id': journal.id, 'date': dt_str,
                    'memo': f"PAY_GDR/{sheet_pref}/{vref} | {narration}"[:255],
                })
                
                # 3. SQL Link + CACHE INVALIDATION
                # This bypasses Odoo's _check_move_id constraint during commit by making the record correct in DB
                env.cr.execute("UPDATE account_payment SET move_id = %s, state = 'in_process' WHERE id = %s", (move.id, payment.id))
                env.cr.execute("UPDATE account_move SET origin_payment_id = %s WHERE id = %s", (payment.id, move.id))
                
                # SYNC ODOO CACHE with SQL UPDATES
                payment.invalidate_recordset(['move_id', 'state'])
                move.invalidate_recordset(['origin_payment_id'])
