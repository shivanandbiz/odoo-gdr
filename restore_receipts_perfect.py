        ...
        try:
            if is_transfer:
                ...
                payment = env['account.payment'].create({
                    'payment_type': 'internal', 'date': dt_str, 'amount': amount,
                    'journal_id': source_journal.id, 'destination_journal_id': journal.id,
                ...
                # We need to create a manual move + SQL bypass to avoid the Odoo issue
                ...
                move = env['account.move'].create({
                    ...
                    'line_ids': [
                        (0, 0, {'name': f"Receipt: {part_name} | {narration}", 'account_id': dest_acc_id, 'debit': amount}),
                        (0, 0, {'name': f"Receipt: {part_name} | {narration}", 'account_id': source_acc_id, 'partner_id': partner.id, 'credit': amount}),
                    ]
                })
                ...
                env.cr.execute("""
                    ...
                    RETURNING id
                """, (amount, dt_str, journal.id, partner.id, 'inbound', 'customer', 'in_process', memo, move.id, journal.company_id.id, journal.currency_id.id or 1, mline_id))
