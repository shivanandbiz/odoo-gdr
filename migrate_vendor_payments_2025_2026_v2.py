...
def migrate_payments(env):
        ...
        for ridx, row in data_df.iterrows():
            ...
            try:
                with env.cr.savepoint():
                    ...
                    particulars = str(row[col_part]).strip()
                    ...
                    if not particulars or particulars.lower() in ['nan', '', 'none'] or 'total' in particulars.lower():
                        continue
                    ...
                    if is_particulars_bank:
                        source_name = particulars
                        # Look for target in account columns
                        ...
                        # Look for source in account columns
                        for c_idx, acc_name in account_cols.items():
                    ...
                    if not source_name:
                        # Fallback for source if not found
                        if 'cash' in particulars.lower(): source_name = 'CASH'
                        else: source_name = "HDFC C/A" 

                    if not target_name:
                        # If target_name is still empty, maybe particulars was a bank and no column had value?
                        # Or vice versa. Use particulars as target if all else fails.
                        target_name = particulars

                    journal = get_journal(env, source_name)
                    if not journal:
                        print(f"Error: No journal found for {source_name} at row {ridx}")
                        total_errors += 1
                        continue
                    ...
                    env.cr.execute("""
                        ...
                        RETURNING id
                    """, (gross_total, dt_str, journal.id, partner.id if partner else None, 'outbound', 'supplier', 'posted', narration[:255], move.id, journal.company_id.id, journal.currency_id.id or 1, mline.id if mline else None))
    ...
    print(f"Total Payments Skipped (Already Migrated): {total_skipped}")
    print(f"Total Payments Reconciled: {total_reconciled}")
