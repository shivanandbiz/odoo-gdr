...
class GSTR2BReportXlsx(models.AbstractModel):
    _name = 'report.l10n_in_gstr2b_report.gstr2b_report_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    _description = 'GSTR-2B Reconciliation Excel'
    ...
    def generate_xlsx_report(self, workbook, data, wizard):
        ...
        row += 1
        sheet.write(row, 0, 'GSTR-2B Reconciliation', title_fmt)
        row += 1
        ...
        # Part A Available
        sheet.write(row, 0, "Input Tax Credit Available - Part A", bold_label_format)
        row += 1
        
        rcm_moves = in_invoices.filtered(lambda m: any(tax.l10n_in_reverse_charge for line in m.invoice_line_ids for tax in line.tax_ids))
        imp_goods = in_invoices.filtered(lambda m: m.l10n_in_gst_treatment == 'overseas')
        other_itc = in_invoices.filtered(lambda m: m.partner_id.vat and m.id not in rcm_moves.ids and m.id not in imp_goods.ids)
        
        row, d1 = write_itc_row(row, "All other ITC from Registered Persons (Excluding Reverse Charge)", other_itc)
        row, d2 = write_itc_row(row, "Inward Supplies from ISD", self.env['account.move'])
        row, d3 = write_itc_row(row, "Inward Supplies Liable for Reverse Charge", rcm_moves)
        row, d4 = write_itc_row(row, "Import of Goods", imp_goods)
        row, d5 = write_itc_row(row, "Reversal of Available ITC (Purchase Return) - Part B", self.env['account.move'])
        row, d6 = write_itc_row(row, "Others", self.env['account.move'])
        
        # Total Available
        total_d = tuple(sum(x) for x in zip(d1, d2, d3, d4, d5, d6))
        sheet.write(row, 0, "Total Available ITC", bold_label_format)
        ...
        # Part A Unavailable
        sheet.write(row, 0, "Input Tax Credit Unavailable - Part A", bold_label_format)
        row += 1
        row, _ = write_itc_row(row, "All other ITC from Registered Persons (Excluding Reverse Charge)", self.env['account.move'])
        row, _ = write_itc_row(row, "Inward Supplies from ISD", self.env['account.move'])
        row, _ = write_itc_row(row, "Inward Supplies Liable for Reverse Charge", self.env['account.move'])
        row, _ = write_itc_row(row, "Reversal of Unavailable ITC (Purchase Return) - Part B", self.env['account.move'])
        row, _ = write_itc_row(row, "Others", self.env['account.move'])
        sheet.write(row, 0, "Total Unavailable ITC", bold_label_format)
        row += 1
