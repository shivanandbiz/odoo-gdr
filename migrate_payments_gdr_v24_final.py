...
def migrate_gdr_payments():
    ...
    env.cr.execute("DELETE FROM account_move WHERE ref LIKE 'PAY_GDR/%'")
    env.cr.commit()
    print("Cleanup done.")
    ...
    for sheet in sheet_names:
        ...
        for idx, row in df.iloc[data_start_idx:].iterrows():
            ...
            try:
                ...
                try:
                    ...
                    print(f"  Row {idx} skip: post fail")
                    continue

                # 2. SQL Insert for Payment (Safe)
                # Find method line id via ORM first to avoid SQL collisions
        ...
        env.cr.commit()
    print(f"\nFINAL: {total_count} Total | {total_reconciled} Reconciled | {errors} Errors")
