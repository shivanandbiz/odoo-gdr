...
def migrate_gdr_payments():
    ...
    env.cr.execute("""
        ...
        AND move_type = 'entry' AND (ref IS NULL OR ref = '' OR ref LIKE '%Register%')
    """)
    # (Unlink entries)
    env.cr.commit()
    ...
    for sheet in sheet_names:
        ...
        for idx, row in df.iloc[data_start_idx:].iterrows():
            ...
            try:
                ...
                # 1. Create Posted Move (This ensures accounting correct)
                move = env['account.move'].create({
                    'move_type': 'entry', 'date': dt_str, 'ref': f"PAY_GDR/{sheet_pref}/{vref}",
                ...
                # 2. Create Draft Payment (This ensures visibility in menu)
                ...
                # 3. SQL Finalization (The magic link)
